function outputs = OCP_tempchange_fmincon_par(input,filename)
...
for k = 1:nrun
    ...
    [Tb(:,k), fval(k)] = fmincon(@(Tb)obj_tempchange_pl(Tb, tb, tspan, input.dTdt_obj, input.IC, input.option_ode2, input), ...
        Tb_ini, [], [], [], [], Tb_lb, Tb_ub,[],input.optfminconpar);
    twall(k) = toc;
end
